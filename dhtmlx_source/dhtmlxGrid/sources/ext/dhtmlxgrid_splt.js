...
dhtmlXGridObject.prototype.splitAt=function(ind){
    ...
    for (var i=0; i<ind; i++){
        ...
        if (_isFF) b_ha[1][i]=b_ha[1][i]*1+2;
...
//inner methods
		...
		this.loadSizeFromCookie=function(name){
			...
			this.getColWidth = function(i){
				if  (i<ind) return parseInt(this._fake.cellWidthPX[i])+((_isFF)?2:0);
				else return parseInt(this.cellWidthPX[i])+((_isFF)?2:0);
            }
			this.setColWidthA=this._fake.setColWidthA=this.setColWidth;
			this.setColWidth = function(i,value){
				i=i*1;
				if  (i<ind) this._fake.setColWidthA(i,value-((_isFF) ? 2 : 0));
				else this.setColWidthA(i,value-((_isFF) ? 2 : 0));
